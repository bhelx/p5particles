 ...
 * References:
 * 
 * http://www.particle.kth.se/~lindsey/JavaCourse/Book/Part1/Physics/Chapter04/
 * RungeKutta4th.html
 * 
 * @author bhelx
 */
public class RungeKuttaFourthOrder extends ODESolver {
	...
	public void stepBy(float h) {
		...
		for (int i = 0; i < numParticles; i++) {
			...
			if (!particle.fixed) {
				particle.setLoc(PVector.add(originalPositions[i],
						PVector.mult(k1Vels[i], halfH)));
				particle.setVel(PVector.add(originalVelocities[i], PVector.div(
						PVector.mult(k1Accs[i], halfH), particle.mass)));
			}
		}
		...
		for (int i = 0; i < numParticles; i++) {
			...
			if (!particle.fixed) {
				particle.setLoc(PVector.add(originalPositions[i],
						PVector.mult(k2Vels[i], halfH)));
				particle.setVel(PVector.add(originalVelocities[i], PVector.div(
						PVector.mult(k2Accs[i], halfH), particle.mass)));
			}
		}
		...
		for (int i = 0; i < numParticles; i++) {
			...
			if (!particle.fixed) {
				particle.setLoc(PVector.add(originalPositions[i],
						PVector.mult(k3Vels[i], h)));
				particle.setVel(PVector.add(originalVelocities[i],
						PVector.div(PVector.mult(k3Accs[i], h), particle.mass)));
			}
		}
		...
		for (int i = 0; i < numParticles; i++) {
			...
			if (!particle.fixed) {
				...
				final PVector kVelSum = PVector.add(
						PVector.add(PVector.add(k1Vels[i], k2Vel), k3Vel),
						k4Vels[i]);
				kVelSum.mult(h / 6f);
				...
				final PVector kAccSum = PVector.add(
						PVector.add(PVector.add(k1Accs[i], k2Acc), k3Acc),
						k4Accs[i]);
				kAccSum.mult(h / 6f);
